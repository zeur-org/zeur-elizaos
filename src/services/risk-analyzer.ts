...
export class RiskAnalyzer {
    ...
    async assessPortfolioRisk(protocols: StakingProtocol[]): Promise<RiskAssessment> {
    ...
    private calculateConcentrationRisk(protocols: StakingProtocol[]): number {
        ...
        const totalTVL = protocols.reduce((sum, p) => sum + Number(p.totalStaked), 0);
    ...
    private calculateLiquidityRisk(protocols: StakingProtocol[]): number {
        ...
        const totalStaked = protocols.reduce((sum, p) => sum + Number(p.totalStaked), 0);
    ...
    private calculateSmartContractRisk(protocols: StakingProtocol[]): number {
        ...
        for (const protocol of protocols) {
            ...
            if (protocol.name === 'lido') contractRisk = 1; // Most mature
    ...
    private generateRecommendations(
        ...
        if (overallRisk > 7) {
            ...
            recommendations.push("Very low risk - consider taking on more yield opportunities");
        }
        
        // Concentration risk
        if (concentrationRisk > 6) {
            recommendations.push("Portfolio too concentrated - diversify across more protocols");
        }
        
        // Liquidity risk
        if (liquidityRisk > 5) {
            recommendations.push("High liquidity risk - increase allocation to protocols with shorter withdrawal delays");
        }
        
        // Smart contract risk
        if (smartContractRisk > 6) {
            recommendations.push("High smart contract risk - reduce exposure to newer protocols");
        }
        
        // Protocol-specific recommendations
        const highRiskProtocols = protocols.filter(p => this.calculateProtocolRisk(p) > 7);
        if (highRiskProtocols.length > 0) {
            recommendations.push(`Consider reducing exposure to high-risk protocols: ${highRiskProtocols.map(p => p.name).join(', ')}`);
        }
        ...
        if (totalStaked > 0) {
            for (const protocol of protocols) {
                ...
                if (allocation > protocol.maxAllocation / 100) {
                    recommendations.push(`${protocol.name} allocation (${(allocation * 100).toFixed(1)}%) exceeds recommended maximum (${protocol.maxAllocation}%)`);
                }
            }
        }
    ...
    async calculateVaR(
        ...
        const zScore = confidenceLevel === 0.95 ? 1.65 :
                      confidenceLevel === 0.99 ? 2.33 :
                      confidenceLevel === 0.90 ? 1.28 : 1.65;
    ...
    async stressTest(
        ...
        for (const scenario of scenarios) {
            let totalImpact = 0;
            let totalStaked = protocols.reduce((sum, p) => sum + Number(p.totalStaked), 0);
