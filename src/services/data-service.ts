...
export class DataService {
    ...
    async getMarketData(): Promise<MarketData> {
        ...
        try {
            ...
            const [gasPrice, ethPrice, protocolAPRs, protocolRisks] = await Promise.all([
                this.getGasPrice(),
                this.getETHPrice(),
                this.getProtocolAPRs(),
                this.getProtocolRisks()
            ]);
        ...
        try {
            ...
            for (const [protocol, searchTerms] of Object.entries(protocolMappings)) {
                ...
                if (pool) {
                    ...
                    // Fallback APRs
                    const fallbacks = { lido: 8.1, rocketpool: 7.9, etherfi: 8.4, morpho: 9.2 };
                    aprs.set(protocol, fallbacks[protocol as keyof typeof fallbacks] || 8.0);
                }
            }
    ...
    async getHistoricalData(protocol: string, timeframe: string): Promise<any[]> {
        ...
        const mockData = [];
        const now = Date.now();
        const interval = timeframe === '24h' ? 3600000 : // 1 hour
                        timeframe === '7d' ? 3600000 * 6 : // 6 hours
