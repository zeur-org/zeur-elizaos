...
export class BlockchainService {
    ...
    private initializeContracts() {
        ...
        if (process.env.VAULT_ETH_ADDRESS) {
            this.contracts.set('vaultETH', new ethers.Contract(
        ...
        for (const [name, address] of Object.entries(routerAddresses)) {
            if (address) {
                this.contracts.set(name, new ethers.Contract(
                    address,
                    stakingRouterABI,
                    this.wallet
                ));
    ...
    async getProtocolData(protocolName: string): Promise<Partial<StakingProtocol> | null> {
        try {
            ...
            const [stakedToken, exchangeRate] = await contract.getStakedTokenAndExchangeRate();
    ...
    async executeStake(protocolName: string, amount: bigint): Promise<string | null> {
        try {
            const vaultETH = this.contracts.get('vaultETH');
            if (!vaultETH) return null;
    ...
    async executeUnstake(protocolName: string, amount: bigint): Promise<string | null> {
        try {
            const vaultETH = this.contracts.get('vaultETH');
            if (!vaultETH) return null;
    ...
    async waitForTransaction(txHash: string, timeout: number = 300000): Promise<boolean> {
        try {
