...
export const YieldAnalyticsAction: Action = {
  ...
  description:
    "Analyzes yield performance, tracks metrics, and provides detailed analytics",
    ...
    try {
      ...
      const analytics = {
        ...
        protocolPerformance: {
          ...
          rocketpool: { allocation: 24.1, apr: 7.9, contribution: 1.9 },
      ...
      for (const [period, data] of Object.entries(analytics.timeframes)) {
        const outperformance = (
          ((data.return - data.benchmark) / data.benchmark) *
          100
        ).toFixed(2);
